...
/**
 * Protects private credential objects belonging to a {@code Subject}. It has only one action which
 * is "read". The target name of this permission has a special syntax:
 *
 * <pre>
 * targetName = CredentialClass {PrincipalClass &quot;PrincipalName&quot;}*
 * </pre>
 *
 * First it states a credential class and is followed then by a list of one or more principals
 * identifying the subject.
 * <p>
 * The principals on their part are specified as the name of the {@code Principal} class followed by
 * the principal name in quotes. For example, the following file may define permission to read the
 * private credentials of a principal named "Bob":
 * "com.sun.PrivateCredential com.sun.Principal \"Bob\""
 * <p>
 * The syntax also allows the use of the wildcard "*" in place of {@code CredentialClass} or
 * {@code PrincipalClass} and/or {@code PrincipalName}.
 *
 * @see Principal
 */
public final class PrivateCredentialPermission extends Permission {
    ...
    /**
     * Creates a new permission for private credentials specified by the target name {@code name}
     * and an {@code action}. The action is always {@code "read"}.
     *
    ...
    /**
     * Creates a {@code PrivateCredentialPermission} from the {@code Credential} class and set of
     * principals.
     * 
    ...
    /**
     * Initialize a PrivateCredentialPermission object and checks that a target name has a correct
     * format: CredentialClass 1*(PrincipalClass "PrincipalName")
     */
     ...
     * {@code PrivateCredentialPermission} as a two dimensional array. The first dimension of the
     * array corresponds to the number of principals. The second dimension defines either the name
     * of the {@code PrincipalClass} [x][0] or the value of {@code PrincipalName} [x][1].
     * <p>
     ...
     * </pre>
     *
     * @return the principal classes and names associated with this
    ...
    /**
     * Returns true if the two arrays have the same length, and every member of one array is
