...
/**
 * Merges permissions based on code source and code signers with permissions granted to the
 * specified {@link Subject}.
 */
public class SubjectDomainCombiner implements DomainCombiner {
    ...
    /**
     * Merges the {@code ProtectionDomain} with the {@code Principal}s associated with the subject
     * of this {@code SubjectDomainCombiner}.
     *
     * @param currentDomains the {@code ProtectionDomain}s associated with the context of the
     *            current thread. The domains must be sorted according to the execution order, the
     *            most recent residing at the beginning.
     * @param assignedDomains the {@code ProtectionDomain}s from the parent thread based on code
     *            source and signers.
     * @return a single {@code ProtectionDomain} array computed from the two provided arrays, or
     *         {@code null}.
            ...
            for (cur = 0; cur < currentDomains.length; cur++) {
                if (currentDomains[cur] != null) {
                    ...
                    newPD = new ProtectionDomain(currentDomains[cur].getCodeSource(),
                            currentDomains[cur].getPermissions(),
