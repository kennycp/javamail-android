...
package cirruspath.javax.security.auth;
...
/**
 * The central class of the {@code javax.security.auth} package representing an authenticated user
 * or entity (both referred to as "subject"). IT defines also the static methods that allow code to
 * be run, and do modifications according to the subject's permissions.
 * <p>
 * A subject has the following features:
 * <ul>
 * <li>A set of {@code Principal} objects specifying the identities bound to a {@code Subject} that
 * distinguish it.</li>
 * <li>Credentials (public and private) such as certificates, keys, or authentication proofs such as
 * tickets</li>
 * </ul>
 */
public final class Subject implements Serializable {
    ...
    /**
     * The default constructor initializing the sets of public and private credentials and
     * principals with the empty set.
     */
    public Subject() {
        super();
    ...
    /**
     * The constructor for the subject, setting its public and private credentials and principals
     * according to the arguments.
     *
     * @param readOnly {@code true} if this {@code Subject} is read-only, thus preventing any
     *            modifications to be done.
     * @param subjPrincipals the set of Principals that are attributed to this {@code Subject}.
     * @param pubCredentials the set of public credentials that distinguish this {@code Subject}.
     * @param privCredentials the set of private credentials that distinguish this {@code Subject}.
     */
    ...
    /**
     * Run the code defined by {@code action} using the permissions granted to the {@code Subject}
     * and to the code itself, additionally providing a more specific context.
     *
     * @param subject the distinguished {@code Subject}.
     * @param action the code to be run.
    ...
    /**
     * Runs the code defined by {@code action} using the permissions granted to the subject and to
     * the code itself.
     *
     * @param subject the distinguished {@code Subject}.
     * @param action the code to be run.
    ...
    /**
     * Runs the code defined by {@code action} using the permissions granted to the subject and to
     * the code itself, additionally providing a more specific context.
     *
     * @param subject the distinguished {@code Subject}.
     * @param action the code to be run.
     * @param context the specific context in which the {@code action} is invoked. if {@code null} a
    ...
    public static Object doAsPrivileged(Subject subject, PrivilegedExceptionAction action,
        ...
        if (context == null) {
            return doAs_PrivilegedExceptionAction(subject, action, new AccessControlContext(
                    new ProtectionDomain[0]));
        }
    ...
    /**
     * Checks two Subjects for equality. More specifically if the principals, public and private
     * credentials are equal, equality for two {@code Subjects} is implied.
     *
     * @param obj the {@code Object} checked for equality with this {@code Subject}.
     * @return {@code true} if the specified {@code Subject} is equal to this one.
     */
    @Override
    public boolean equals(Object obj) {
        ...
        if (principals.equals(that.principals) && publicCredentials.equals(that.publicCredentials)
    ...
    /**
     * Returns this {@code Subject}'s {@link Principal} which is a subclass of the {@code Class}
     * provided.
     *
     * @param c the {@code Class} as a criteria which the {@code Principal} returned must satisfy.
     * @return this {@code Subject}'s {@link Principal}. Modifications to the returned set of
     *         {@code Principal}s do not affect this {@code Subject}'s set.
     */
     ...
     * setting it to read-only this {@code Subject} can not be made writable again. The destroy
     * method on the credentials still works though.
     */
    ...
    private final class SecureSet<SST> extends AbstractSet<SST> implements Serializable {
            ...
            if (permission == _PRIVATE_CREDENTIALS) {
                ...
                return new SecureIterator(elements.iterator()) {
                    ...
                    public SST next() {
                        ...
                        checkPermission(new PrivateCredentialPermission(obj.getClass().getName(),
                                principals));
                        return obj;
                    }
                };
        ...
        private class SecureIterator implements Iterator<SST> {
            ...
            /**
             * checks set state, and security permission to modify set before removing current
             * element
             */
            public void remove() {
                checkState();
